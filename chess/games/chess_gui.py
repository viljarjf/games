from chess.games.abstract.chess import Chess
...
class ChessGUI(Chess):
    ...
    def __init__(self, corner, sidelength):
        ...
        self._id_board = np.array([None for _ in np.zeros(shape).flatten()]).reshape(shape)
        self._root = tk.Tk()
        ...
        self.pad = 10
        self._tile_size = (sidelength - 2*self.pad)//(self.board_size)
    ...
    def draw_tile(self, pos: tuple)-> bool:
        ...
        Args:
            pos (tuple):    coordinates for the tile. 
    ...
    def pixel_from_pos(self, pos)-> tuple:
        ...
        Args:
            pos (tuple):    4D coordinates for the tile
